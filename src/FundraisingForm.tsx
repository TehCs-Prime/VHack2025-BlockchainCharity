...
const FundraisingForm: React.FC = () => {
  ...
  const handleInputSentences = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (wordCount <= 20) {
      ...
      event.target.value = formData.projectQuote;
    }
  };

  const handleQuoteKeyDown = (event: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (wordCount >= 20 && !specialKeys.includes(event.key) && !event.metaKey && !event.ctrlKey) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setIsSubmitted(true);
      console.log('Complete form submitted:', finalFormData);
    } catch (error) {
    ...
    return (
      <div className="fundraising-success">
        <div className="text-center">
          ...
          <small style={{opacity:"0.5"}}>This proposal is under review and verification process. <br></br>Please wait as this process usually takes less than 24 hours<br></br></small>
          <button
            onClick={resetForm}
          ...
          <form onSubmit={handleContinueToMilestones} className="fundraising-form">
            ...
            <div className="form-group">
              <label htmlFor="projectQuote" className="form-label">
                Project Quote * <span style={{ fontSize: '0.8rem', color: wordCount >= 20 ? 'red' : 'inherit' }}>
                  ({wordCount}/20 words)
                </span>
              </label>
              <textarea
